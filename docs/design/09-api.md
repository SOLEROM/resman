...
| GET | `/api/vaults/{name}/wiki/tree` | Walk `<vault>/wiki/` recursively, returning sorted dirs + `.md` files for the Wiki tab's sidebar tree. Paths are vault-relative (e.g. `wiki/sources/foo.md`). Each file node carries `unread` and `favorite` flags. Hidden entries and symlinks are skipped. Returns `{missing: true, tree: []}` when the vault has no `wiki/` directory yet (fresh / pre-bootstrap) |
| GET | `/api/vaults/{name}/wiki/favorites` | Parse `<vault>/.favorites.md` → `{file: ".favorites.md", exists, favorites: [{file, title, exists}]}` in file order. Entry `file` is vault-relative; entry `exists` is false for a dangling link. See [16-wiki-favorites.md](16-wiki-favorites.md) |
| POST | `/api/vaults/{name}/wiki/favorites` | `{file, favorite: bool}` — append to / remove from `.favorites.md` (idempotent, atomic rewrite that preserves hand-written lines). 400 on an unsafe path, 404 when favoriting a page that doesn't exist; removal always succeeds. Returns `{file, favorite, favorites}`. CSRF required |
