...
def test_load_missing_system_yaml_raises(cfg_dir):
    bus = EventBus()
...
def test_load_valid_system_yaml(cfg_dir):
...
def test_save_emits_config_reloaded(cfg_dir):
    ...
    assert received and received[0]["file"] == "system.yaml"
    assert cm.get_vault("beta") is not None
    # File on disk reflects the change
    assert "beta" in (cfg_dir / "system.yaml").read_text()


def test_save_too_large_rejected(cfg_dir):
    write(cfg_dir / "system.yaml", "vaults: []\n")
    cm = ConfigManager(cfg_dir, EventBus())
    cm.load()
    big = "vaults: []\n# " + ("a" * (1024 * 1024 + 10))
...
def test_atomic_write_creates_no_partial_on_error(cfg_dir, monkeypatch):
    ...
    assert (cfg_dir / "system.yaml").read_text() == "vaults: []\n"
...
def test_add_vault(cfg_dir):
    ...
    assert "name: b" in (cfg_dir / "system.yaml").read_text()
